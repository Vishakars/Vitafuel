...
@router.get("/{email}")
async def list_mental(email: str, current_user: dict = Depends(get_current_user)):
    db = get_db()
...
@router.post("/{email}")
async def create_mental(email: str, payload: MentalCreate, current_user: dict = Depends(get_current_user)):
    db = get_db()
    users = db.users
...
async def update_mental(email: str, entry_id: str, payload: MentalUpdate, current_user: dict = Depends(get_current_user)):
    db = get_db()
    users = db.users
    user = await users.find_one({"email": email.strip().lower()})
    if not user:
...
async def delete_mental(email: str, entry_id: str, current_user: dict = Depends(get_current_user)):
    db = get_db()
    users = db.users
    user = await users.find_one({"email": email.strip().lower()})
    if not user:
...
@router.get("/me")
async def list_mental_me(current_user: dict = Depends(get_current_user)):
    db = get_db()
...
@router.post("/me")
async def create_mental_me(payload: MentalCreate, current_user: dict = Depends(get_current_user)):
    db = get_db()
    users = db.users
