...
@router.get("/me")
async def list_diabetes_me(current_user: dict = Depends(get_current_user)):
    db = get_db()
...
@router.get("/{email}")
async def list_diabetes(email: str, current_user: dict = Depends(get_current_user)):
    db = get_db()
...
@router.post("/me")
async def create_diabetes_me(payload: DiabetesCreate, current_user: dict = Depends(get_current_user)):
    db = get_db()
    users = db.users
...
@router.post("/{email}")
async def create_diabetes(email: str, payload: DiabetesCreate, current_user: dict = Depends(get_current_user)):
    db = get_db()
    users = db.users
...
async def update_diabetes(email: str, entry_id: str, payload: DiabetesUpdate, current_user: dict = Depends(get_current_user)):
    db = get_db()
    users = db.users
    user = await users.find_one({"email": email.strip().lower()})
    if not user:
...
async def delete_diabetes(email: str, entry_id: str, current_user: dict = Depends(get_current_user)):
    db = get_db()
    users = db.users
    user = await users.find_one({"email": email.strip().lower()})
    if not user:
