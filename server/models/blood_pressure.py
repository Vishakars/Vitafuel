...
@router.post("/{email}")
async def create_bp(email: str, payload: BPCreate):
    db = get_db()
    users = db.users
...
async def update_bp(email: str, entry_id: str, payload: BPUpdate):
    db = get_db()
    users = db.users
    user = await users.find_one({"email": email.strip().lower()})
    if not user:
...
async def delete_bp(email: str, entry_id: str):
    db = get_db()
    users = db.users
    user = await users.find_one({"email": email.strip().lower()})
    if not user:
